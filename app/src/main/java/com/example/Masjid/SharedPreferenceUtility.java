...
public class SharedPreferenceUtility {
     ...
     */
    public static List<Masjid> getAllFavorite(Context ctx) {
        String jsonString = getSharedPref(ctx).getString(FAVOR_KEY, null);
        List<Masjid> trs = new ArrayList<Masjid>();
        if (jsonString != null) {
            ...
            try {
                ...
                for (int i=0;i<jsonArray.length();i++) {
                    ...
                    trs.add(Masjid.fromJSONObject(obj));
                }
    ...
    /*
        ...
        Perhatikan bahwa data disimpan dalam format JSON String
     */
    private static void saveAllFavorite(Context ctx, List<Masjid> trs) {
     ...
     */
    public static void addFavorite(Context ctx, Masjid tr) {
        List<Masjid> trs = getAllFavorite(ctx);
        trs.add(tr);
        saveAllFavorite(ctx,trs);
    }

    public static void editFavorite(Context ctx, Masjid tr) {
        List<Masjid> trs = getAllFavorite(ctx);
        for (Masjid tre:trs) {
